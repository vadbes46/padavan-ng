...
static char post_buf[1048576] = {0};
...
ej_dump(int eid, webs_t wp, int argc, char **argv)
{
	...
	else if (strncmp(file, "zapret2c.", 9)==0) {
		if (strncmp(file+9, "user.list", 9)==0 || strncmp(file+9, "auto.list", 9)==0 || strncmp(file+9, "exclude.list", 12)==0)
			snprintf(filename, sizeof(filename), "%s/%s", STORAGE_ZAPRET_DIR, file+9);
		else
			snprintf(filename, sizeof(filename), "%s/%s", STORAGE_ZAPRET2_DIR, file+9);
	}
...
validate_asp_apply(webs_t wp, int sid)
{
	...
	for (v = GetVariables(sid); v->name != NULL; ++v) {
		...
		if (!strcmp(v->longname, "File")) {
			...
			else if (!strncmp(v->name, "zapret2c.", 9)) {
				const char *dir = STORAGE_ZAPRET2_DIR;
				if (!strncmp(file_name, "user.list", 9) || !strncmp(file_name, "auto.list", 9) || !strncmp(file_name, "exclude.list", 12))
					dir = STORAGE_ZAPRET_DIR;
				if (write_textarea_to_file(value, dir, file_name))
					restart_needed_bits |= event_mask;
			}
#endif
...
#endif
#if defined(APP_ZAPRET2)
	int found_app_zapret2 = 1;
#else
	int found_app_zapret2 = 0;
#endif
#if defined(APP_TOR)
	int found_app_tor = 1;
#else
...
//	int has_5g_band_steering = 0;
	...
	websWrite(wp,
		...
		"function found_app_zapret2() { return %d;}\n"
