...
static inline u32 wg_range_rand_inclusive(u32 floor, u32 ceil)
{
	u32 diff;
	if (ceil <= floor)
		return floor;
...
{
	return wg_range_rand_inclusive(u32_range_lo(range), u32_range_hi(range));
}
...
{
	return (u16)wg_range_rand_inclusive(u16_range_lo(range), u16_range_hi(range));
}
